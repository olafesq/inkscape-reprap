...
class GCodeContext:
    def __init__(self, xy_feedrate, z_feedrate, start_delay, stop_delay, pen_up_height, z_height, finished_height, x_home, y_home, register_pen, num_pages, continuous, file):
      ...
      self.postscript = [
        "",
				";(end of print job)",
				"G1 Z%0.2F ;(pen up)" % self.pen_up_height,
      ...
      self.registration = [
        "G1 Z%d ;(pen down)" % (self.z_height),
        "G4 P%d ;(wait %dms)" % (self.start_delay, self.start_delay),
        "G1 Z%d ;(pen up)" % (self.pen_up_height),
        "G4 P%d ;(wait %dms)" % (self.stop_delay, self.stop_delay),
    ...
    def start(self):
      self.codes.append("G1 Z%0.2F ;(pen down)" % self.z_height)
      self.codes.append("G4 P%d ;(wait %dms)" % (self.start_delay, self.start_delay))
      self.drawing = True

    def stop(self):
      self.codes.append("G1 Z%0.2F ;(pen up)" % self.pen_up_height)
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            self.codes.append("G1 Z%0.2F ;(pen down)" % self.pen_up_height)
            self.codes.append("G4 P%d ;(wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True
